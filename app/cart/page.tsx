...
export default function ProductsPage() {
    ...
    if(userId){
        ...
        if(res?.success) {
            ...
            if (cart?.items?.length > 0) {
                const hasFrames = cart.items.some((item: any) => item.productType === 'Frame' || !item.productType);
                const hasLenses = cart.items.some((item: any) => item.productType === 'ContactLens');
  ...
  const framesItems = currentUserCart.items.filter((item: any) => item.productType === 'Frame' || !item.productType);
  const lensesItems = currentUserCart.items.filter((item: any) => item.productType === 'ContactLens');
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="flex bg-gray-200/50 p-1.5 rounded-full w-fit mb-6">
                ...
                >
                    Contact Lenses ({lensesItems.length})
                </button>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-24">
                ...
                <div className="space-y-3 mb-6">
                    <div className="flex justify-between text-gray-600">
                        ...
                        <span className="font-medium text-gray-900">{currentUserCart?.items?.length}</span>
                    </div>
                    <div className="flex justify-between text-gray-600">
                        <span>Subtotal</span>
                        <span className="font-medium text-gray-900">₹{currentUserCart?.cartTotal}</span>
                    </div>
                ...
                <div className="flex justify-between items-center mb-8 pt-4 border-t">
                    ...
                    <span className="text-2xl font-bold text-gray-900">₹{currentUserCart?.cartTotal}</span>
                </div>
